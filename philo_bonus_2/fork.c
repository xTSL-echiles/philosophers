...
void	ft_eating(t_tr_philos *tr_phil)
{
	...
	sem_post(tr_phil->phil->sem_f);
	ft_sem_printf(tr_phil, "is sleeping\n");
	usleep_modified((tr_phil->phil->time_to_sleep));
	ft_sem_printf(tr_phil, "is thinking\n");
	if (tr_phil->phil->all_need_eat > 0)
...
void	ft_waiting(t_tr_philos *tr_phil)
{
	...
	j = -1;
	while (++j < tr_phil->phil->num_of_phil)
...
void	ft_fork_start(t_tr_philos *tr_phil, int i)
{
	...
	if (tr_phil->phil->pid[i] == 0)
	{
		...
		while (tr_phil->phil->all_need_eat)
			ft_eating(tr_phil);
		pthread_detach(kil);
		exit(0);
	}
	if (i == tr_phil->phil->num_of_phil - 1)
		ft_waiting(tr_phil);
}
